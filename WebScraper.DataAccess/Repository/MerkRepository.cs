    ...
    public class MerkRepository : IMerk
    {
        ...
        public List<SP_MerkByKotaID> GetMerkByKotaID(long KotaID)
        {
            if (db != null)
            {
                try
                {
                    return db.Set<SP_MerkByKotaID>().FromSqlRaw("EXEC sp_MerkByKotaID {0}",
        ...
        public async Task<List<SP_MerkRank>> GetMerkRank(string search, int draw, int startRec, int pageSize)
        {
            try
            {
                if (db != null)
                {
                    try
                    {
                        return db.Set<SP_MerkRank>().FromSqlRaw("EXEC sp_GetMerkRank {0},{1},{2}", startRec, pageSize, search).AsNoTracking().ToList();
                    }
                    catch (Exception ex)
                    {
                        throw ex;
